...
function parseArgs(argv) {
  ...
  while (i < argv.length) {
    ...
    else if ((a === '--selector' || a === '-s') && argv[i + 1]) { args.selector = argv[++i]; }
    else if (a === '--inject')                     { args.inject = true; }
    else if (a === '--title' && argv[i + 1])       { args.title = argv[++i]; }
    else if (a === '--author' && argv[i + 1])      { args.author = argv[++i]; }
    else if (a === '--width' && argv[i + 1])       { args.width = parseFloat(argv[++i]); }
    else if (a === '--height' && argv[i + 1])      { args.height = parseFloat(argv[++i]); }
...
async function runExporter(argv) {
  ...
  if (!htmlSource) {
    ...
    console.error(`${c.cyan}💡  Try:${c.reset} Specify a path to a local HTML file or a URL (e.g. \`dom-to-pptx-exporter slides.html\`).`);
  ...
  if (!isUrl) {
    ...
    if (!fs.existsSync(resolvedInput)) {
      ...
      console.error(`${c.cyan}💡  Try:${c.reset} Check if the file path is correct, or if it is a URL, verify the protocol prefix (http:// or https://).`);
      process.exit(1);
    }
  }
  ...
  console.log(`${c.bold}Mode:      ${c.reset}  ${c.yellow}programmatic${c.reset} (selector: ${c.dim}${exporterOptions.selector}${c.reset})`);
  console.log();
  ...
  try {
    ...
    console.error(`  2. Ensure all dependencies are installed using "${c.bold}pnpm install${c.reset}" (especially "${c.bold}puppeteer${c.reset}").`);
    process.exit(1);
  }
  ...
  try {
    ...
    if (err.message.includes('selector') || err.message.includes('matching')) {
      console.error(`  - Make sure elements matching the CSS selector "${exporterOptions.selector}" exist in the HTML file.`);
      console.error(`  - If the HTML loads content dynamically, verify that the slides are fully rendered when network is idle.`);
    } else if (err.message.includes('puppeteer') || err.message.includes('browser') || err.message.includes('chrome') || err.message.includes('executable')) {
      console.error(`  - Ensure a compatible web browser (Google Chrome, Microsoft Edge, Chromium, or Firefox) is installed on your system.`);
      console.error(`  - On Windows, install Chrome or Edge. On Linux, run "sudo apt install chromium-browser" or install Chrome.`);
      console.error(`  - Alternatively, run with internet access to let dom-to-pptx auto-download a headless Chrome binary.`);
    } else {
...
const isMain = process.argv[1] && (process.argv[1].endsWith('cli-exporter.js') || process.argv[1].endsWith('dom-to-pptx-exporter'));
if (isMain) {
